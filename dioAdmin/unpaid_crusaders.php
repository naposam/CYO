...
<style type="text/css">
    ...
    <body >
       ...
       //                     $query=mysqli_query($mysqli,"SELECT distinct(crusaders.crusader_id),crusaders.*,year(curDate())-year(DOB) as age, diocese.diocese_name,deanery.deanery_name,parish.parish_name,unit.unit_name,annual_registration_paymenttbl.status FROM crusaders LEFT JOIN diocese ON crusaders.diocese = diocese.diocese_id LEFT JOIN deanery on crusaders.deanery=deanery.deanery_id LEFT JOIN parish on crusaders.parish=parish.parish_id LEFT JOIN unit on crusaders.unit=unit.unit_id LEFT JOIN annual_registration_paymenttbl on crusaders.crusader_id=annual_registration_paymenttbl.crusader_id where annual_registration_paymenttbl.status=0 ") or die(mysqli_error($mysqli));
        ...
        $query=mysqli_query($mysqli,"SELECT *,crusaders.crusader_id as id, year(curDate())-year(DOB) as age, diocese.diocese_name,diocese.diocese_id,deanery.deanery_name,parish.parish_name,unit.unit_name,annual_registration_paymenttbl.diocese_id FROM crusaders LEFT JOIN diocese ON crusaders.diocese = diocese.diocese_id LEFT JOIN deanery on crusaders.deanery=deanery.deanery_id LEFT JOIN parish on crusaders.parish=parish.parish_id LEFT JOIN unit on crusaders.unit=unit.unit_id LEFT JOIN annual_registration_paymenttbl on crusaders.crusader_id=annual_registration_paymenttbl.crusader_id where crusaders.crusader_id  NOT IN(select crusader_id from annual_registration_paymenttbl) and crusaders.diocese='$diocese_id'") or die(mysqli_error($mysqli));
             $num_rows=mysqli_num_rows($query);
             $fetch=mysqli_fetch_array($query);
						?>
                            <div class="navbar navbar-inner block-header">
                                <div class="muted pull-left" style="color:green;"><i class="icon-reorder icon-large" ></i><strong>UNPAID CRUSADERS: <span style="color: black; font-weight: bolder; font-family: sans-serif;"><?php echo $fetch['diocese_name'].' '.'DIOCESE';?></span></strong> </div>
                    ...
                    <p style="margin:0;padding:0;font-weight:bold; font-size: 20px;text-align: center;">UNPAID NATIONAL REGISTRATION LIST: <span style="color: black; font-weight: bolder; font-family: sans-serif;"><?php echo $fetch['diocese_name'].' '.'DIOCESE';?></span></p>
