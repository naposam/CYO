...
<body >
        ...
        <div class="container-fluid">
            ...
            // $query=mysqli_query($mysqli,"SELECT DISTINCT(parish.parish_name),parish.*,annual_registration_paymenttbl.status,diocese.diocese_id from parish left JOIN annual_registration_paymenttbl  on annual_registration_paymenttbl.parish_id=parish.parish_id LEFT JOIN diocese on annual_registration_paymenttbl.diocese_id=diocese.diocese_id where annual_registration_paymenttbl.status=1 and diocese.diocese_id='$diocese_id'") or die(mysqli_error($mysqli));
                            $query=mysqli_query($mysqli,"SELECT DISTINCT(parish.parish_name),parish.*,annual_registration_paymenttbl.status,deanery.deanery_id from parish left JOIN annual_registration_paymenttbl  on annual_registration_paymenttbl.parish_id=parish.parish_id LEFT JOIN deanery on annual_registration_paymenttbl.deanery_id=deanery.deanery_id where annual_registration_paymenttbl.status=1 and deanery.deanery_id='$deanery_id'") or die(mysqli_error($mysqli));
