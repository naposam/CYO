	...
	<table cellpadding="0" cellspacing="0" border="1" class="table" id="example">
        ...
        $unit_id=$fetch['unit_id'];
         $parish_id=$fetch['parish_id'];
		   ...
		   $query=mysqli_query($mysqli,"SELECT DISTINCT(unit.unit_name),unit.*,annual_registration_paymenttbl.status ,parish.parish_id from unit left JOIN annual_registration_paymenttbl  on annual_registration_paymenttbl.unit_id=unit.unit_id LEFT JOIN parish on annual_registration_paymenttbl.parish_id=parish.parish_id  where annual_registration_paymenttbl.status=1 and parish.parish_id='$parish_id' ") or die(mysqli_error($mysqli));
