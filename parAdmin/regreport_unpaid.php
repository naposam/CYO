...
<style type="text/css">
    ...
    <body >
        ...
        <div class="container-fluid">
            <div class="row-fluid">
				<?php include('sidebar_upaid_regreport.php'); ?>
                <div class="span9" id="">
                     <div class="row-fluid">
                        <!-- block -->
        ...
        //                   //count depends on the above
        //                   $resultSet=mysqli_query($mysqli,"SELECT *,year(curDate())-year(DOB) as age, diocese.diocese_name,deanery.deanery_name,parish.parish_name,unit.unit_name FROM crusaders LEFT JOIN diocese ON crusaders.diocese = diocese.diocese_id LEFT JOIN deanery on crusaders.deanery=deanery.deanery_id LEFT JOIN parish on crusaders.parish=parish.parish_id LEFT JOIN unit on crusaders.unit=unit.unit_id where crusaders.crusader_id Not in('$crus_id')") or die(mysqli_error($mysqli));
        ...
        $query=mysqli_query($mysqli,"SELECT distinct(crusaders.crusader_id), crusaders.*,year(curDate())-year(DOB) as age, parish.parish_name,unit.unit_name,annual_registration_paymenttbl.status FROM crusaders LEFT JOIN parish on crusaders.parish=parish.parish_id LEFT JOIN unit on crusaders.unit=unit.unit_id LEFT JOIN annual_registration_paymenttbl on crusaders.crusader_id=annual_registration_paymenttbl.crusader_id where annual_registration_paymenttbl.status=0 and annual_registration_paymenttbl.parish_id='$parish_id'") or die(mysqli_error($mysqli));
            $num_rows=mysqli_num_rows($query);
            $fet= mysqli_fetch_array($query);
						?>
                            <div class="navbar navbar-inner block-header">
                                <div class="muted pull-left" style="color:green;"><i class="icon-reorder icon-large" ></i><strong>PENDING FOR APPROVAL LIST: <span style="color: black; font-weight: bolder; font-family: sans-serif;"><?php echo $fetch['parish_name'].' '.'PARISH';?></span></strong> </div>
                                <div class="muted pull-right" style="color:green"><strong><strong>TOTAL CRUSADERS WITH PENDING TRANSACTIONS:</strong><span class="badge badge-info"><?php  echo $num_rows;  ?></span>
								</div>
                            </div>
                    ...
                    <p style="margin:0;padding:0;font-weight:bold; font-size: 20px;text-align: center;">PENDING APPROVAL NATIONAL REGISTRATION LIST OF CRUSADERS: <span style="color: black; font-weight: bolder; font-family: sans-serif;"><?php echo $fetch['parish_name'].' '.'PARISH';?></span></p>
                    <small style="margin:0;padding:0; font-style: italic;">printed on: &nbsp;<?php echo date('d-m-Y')?></small>
                                </h2>
                                    <?php include('regreport_unpaid_table.php'); ?>
                                </div>
                            </div>
                        </div>
                        <!-- /block -->
