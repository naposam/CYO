<?php include('DbCon.php'); ?>
	...
	<table cellpadding="0" cellspacing="0" border="1" class="table" id="example">
        ...
        $unit_id=$fetch['unit_id'];
		   ...
		   $query=mysqli_query($mysqli,"SELECT DISTINCT(unit.unit_name),unit.*,annual_registration_paymenttbl.status  from unit left JOIN annual_registration_paymenttbl  on annual_registration_paymenttbl.unit_id=unit.unit_id  where annual_registration_paymenttbl.status=1 and unit.unit_id='$unit_id' ") or die(mysqli_error($mysqli));
