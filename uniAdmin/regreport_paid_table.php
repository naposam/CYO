...
$sql="SELECT *, unit.unit_name FROM crusaders LEFT JOIN unit on crusaders.unit=unit.unit_id where id='$session_id'";
        ...
        $unit_id=$fetch['unit_id'];
                           ...
                           $query=mysqli_query($mysqli,"SELECT distinct(crusaders.crusader_id),crusaders.*,year(curDate())-year(DOB) as age, unit.unit_name,annual_registration_paymenttbl.status,annual_registration_paymenttbl.period FROM crusaders LEFT JOIN unit on crusaders.unit=unit.unit_id LEFT JOIN annual_registration_paymenttbl on crusaders.crusader_id=annual_registration_paymenttbl.crusader_id where annual_registration_paymenttbl.status=1 and annual_registration_paymenttbl.unit_id='$unit_id' ") or die(mysqli_error($mysqli));
